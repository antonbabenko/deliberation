...
test("CB-MW2: absent consensus.maxWallMs defaults to 1800000", () => {
  const { resolved } = validateConfig(base());
  assert.equal(resolved.consensus.maxWallMs, 1800000);
});

test("CB-MW3: invalid consensus.maxWallMs falls back to 1800000 + a warning", () => {
  for (const bad of [-5, 0, 1.5, "x", true, null]) {
    const c = base();
    c.consensus = { maxWallMs: bad };
    const { resolved } = validateConfig(c);
    assert.equal(resolved.consensus.maxWallMs, 1800000, `${JSON.stringify(bad)}: default`);
    assert.ok(
