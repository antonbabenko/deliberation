...
9. **A provider stub is an error, not an answer** - `agy` can exit 0 having printed only a preamble ("I will begin by finding the repository directory..."). Any non-empty stdout used to count as a full answer, so the stub flowed downstream as a real opinion, and inside consensus `parseReview` turned it into `verdict: null` - silently blocking convergence with no diagnostic. A clean exit shorter than `GEMINI_MIN_ANSWER_CHARS` (default 80, `0` disables) now fails with `errorKind: "empty"`, which `callProvider` retries once. The floor applies only on the normal clean-exit path; the soft-timeout drain branch is untouched, because a short recovered answer still beats a hard timeout. The char count is a proxy for "announced intent instead of answering" - the upgrade path is a structured-output check once `parseOpinion` (today referenced only by tests) is wired into the pipeline. **Grok has the same failure with a different cause**: `grok-4.6` is agentic-trained, and a prompt that mentions repos or "verify the cited files" makes it announce a plan ("I'll verify the cited files...") and end its turn waiting for a tool call that can never come (`tools: []`, `status: completed`, 99-162 chars). The bridge (`server/grok/index.js`, shared by the standalone `grok` tool and the unified server) now (a) always seeds the system turn with a runtime note - no tools, no filesystem, no further turns, answer now - and (b) rejects as `empty` any reply under `GROK_MIN_ANSWER_CHARS` (default 80, `0` disables) or under 400 chars that opens with an intent phrase; an empty body (`""`), previously a silent success, trips the floor too. Details in [TECHNICAL.md § Answer floor](TECHNICAL.md#answer-floor-gemini-grok).
